...
export default function EditarMontadoraPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  async function fetchMontadora() {
    try {
      const res = await fetch(`/api/admin/montadoras/${params.id}`);
      if (res.ok) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(error.error || "Erro ao atualizar montadora");
      }
    } catch (err) {
      alert("Erro ao atualizar montadora");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto p-8">
      <h1 className="text-2xl font-bold mb-4">Editar Montadora</h1>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Digite a descrição da montadora"
          />
        </div>
